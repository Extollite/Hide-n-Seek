...
public class HNSACommand extends CommandManager {

    private final Map<String, Command> commands;

    public HNSACommand() {
        super("hnsa", "", "/hnsa <command>");

        this.setAliases(new String[]{"hidenseekadmin"});
        Permission permission = new Permission("hns.admin.command");
    ...
    public void registerCommand(Command command) {
        this.commands.put(command.getName().replace("hnsa", "").replace("hidenseekadmin", "").toLowerCase(), command);
        this.updateArguments();
    }

    static public void sendUsage(CommandSender sender) {
        sender.sendMessage(TextFormat.GREEN + "-- HNS " + HNS.getInstance().getDescription().getVersion() + " Admin Commands --");
        for(String message : HNS.getInstance().getLanguage().getCmd_admin_usage()){
